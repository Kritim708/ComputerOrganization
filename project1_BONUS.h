/*
    ...
    Description: Decleartion of project1 methods alongside implementation of some utility functions
*/
...
//Utility map for keeping track of label and its respective lines
std::unordered_map<std::string, int> labels_line;
//Utility map for keeping track of of memory label and their respective size
std::unordered_map<std::string, int> labels_memory;
...
int encode_Rtype(int opcode, int rs, int rt, int rd, int shftamt, int funccode) {
    // return (opcode << 26) + (rs << 21) + (rt << 16) + (rd << 11) + (static_cast<int>(shftamt & 0xffff) << 6) + funccode;
...
int encode_Itype(int opcode, int rs, int rt, int const_){
    return (opcode << 26) + (rs << 21) + (rt << 16) + static_cast<int>(const_ & 0xffff);
    // if (const_ < 0){
    //     return (opcode << 26) + (rs << 21) + (rt << 16) + const_;
    // } else {
    //     return (opcode << 26) + (rs << 21) + ((rt+1) << 16) + const_ ;
    // }
    
}

//Method that calculates the decimal equivalent of the two's complement
int twos_complement(int decimal){
    //Changes only for negative values
    if (decimal < 0){
        //Calculates binary
        bitset<16> binary(abs(decimal));
        //Flips the values
        binary.flip();
        //Adds one on it
        return (int) binary.to_ulong() + 1;
    }
    return decimal;
